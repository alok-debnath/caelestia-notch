...
StyledWindow {
    ...
    // The launcher is not in this window any more -- the island holds the
    // keyboard for it, and two layer surfaces both claiming focus for the same
    // state is how you lose a keystroke.
    WlrLayershell.keyboardFocus: screenState.session ? WlrKeyboardFocus.OnDemand : WlrKeyboardFocus.None
    ...
    HyprlandFocusGrab {
        ...
        active: {
            ...
            // Not the launcher: its grab belongs to the island window now, or
            // this one would treat a click on the notch as a click outside.
            if ((s.session && conf.session.enabled) || (s.sidebar && conf.sidebar.enabled))
                return true;
    ...
    Item {
        ...
        // The island's notch. It lives in its own layer-shell window, but that
        // window is placed at the border origin by the exclusion zones and its
        // capsule is positioned relative to it, so the capsule's coordinates are
        // already in the space PanelBg works in.
        //
        // Drawing it here rather than in the island's own window is what makes
        // it fuse into the top border instead of floating below it, and the
        // square top corners are what make it a notch rather than a pill.
        BlobRect {
            id: islandBg

            readonly property var islandWindow: ShellState.componentsFor(root.screen)?.island ?? null
            readonly property Item capsule: islandWindow?.capsule ?? null
